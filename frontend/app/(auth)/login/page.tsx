...
export default function LoginPage() {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      } else if (msg.toLowerCase().includes('network') || msg.toLowerCase().includes('connect') || msg.toLowerCase().includes('unable')) {
        setError('Unable to connect.')
      } else if (msg.includes('reject') || msg.includes('Authentication failed') || msg.includes('failed')) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Brand matched Google OR divider */}
      <div className="relative my-5">
        <div className="absolute inset-0 flex items-center" aria-hidden="true">
          <div className="w-full border-t border-zinc-800/85" />
        </div>
        <div className="relative flex justify-center text-xs uppercase">
          <span className="bg-[#09090b] px-3 text-zinc-500 font-mono font-semibold">Or</span>
        </div>
      </div>
      ...
      <button
        ...
        className="w-full bg-white hover:bg-zinc-100 active:scale-[0.98] text-zinc-900 font-semibold py-2 rounded-lg flex items-center justify-center gap-2.5 transition-all duration-200 shadow-md border border-zinc-200 cursor-pointer"
        disabled={submitting || googleLoading}
      >
        {googleLoading ? (
          <span className="flex items-center justify-center gap-2">
      ...
      <div className="mt-6 border-t border-zinc-800/80 pt-4 text-center">
        <p className="text-[10px] text-zinc-500 leading-relaxed">
          Sandbox Mode active by default. Enter any email and password or continue with Google to log in instantly.
        </p>
      </div>
    </div>
  )
